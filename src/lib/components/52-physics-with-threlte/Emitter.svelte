<script lang="ts">
	...
	import Particle from './SmallBoxes.svelte';
	...
	const getRandomPosition = (): Parameters<Vector3['set']> => {
		return [0.5 - Math.random() * 1, 4, 0.5 - Math.random() * 1];
	};
	...
	useTask(() => {
		if (lastBodyMounted + bodyEveryMilliseconds < Date.now() && bodies.length < MAX_CUBES) {
			const body: Body = {
				id: getId(),
				mounted: Date.now(),
